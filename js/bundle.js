//BRR
let geodataUrl = 'data/mwi_adm2.json';
let fourWDataUrl = 'https://raw.githubusercontent.com/collective-service/cs-kobo-scraper/main/data/data_malawi_4w.csv';
let mwi_dictionaryURL = 'https://raw.githubusercontent.com/collective-service/cs-kobo-scraper/main/data/mw_4w_dictionary.csv';
...
$(document).ready(function() {
    function getData() {
        Promise.all([
            ...
            d3.csv(mwi_dictionaryURL)
        ]).then(function(data) {
            geomData = topojson.feature(data[0], data[0].objects.mwi_adm2);
            mappingData = data[1];
            filteredMappingData = mappingData;
            // console.log(filteredMappingData)
            dataDictionary = data[3];
...
$(".children-selections button").on("click", function(d) {
    ...
    if (buttonName == "select-all") {
        ...
        $(".children li").each(function(index, li) {
            $(li).hasClass('is-selected') ? $(li).removeClass('is-selected') : null;
            $(li).children('.grandchildrenContent').remove();
        });
    }
    // clear filters
    //call reset viz?
...
function getSelectedItemFromUl(className) {
    ...
    items.each(function(idx, li) {
        ...
        const selection = $(this).children('.item').children('h6').text(); //d3.select(this).select(".item").select("h6").text();
        // const val = $(this).children('.item > h6').text();
...
function initiateMap() {
    ...
    height = 80;
    var mapScale = (isMobile) ? width / 5.5 : width * 6.2;
    var mapCenter = (isMobile) ? [12, 12] : [34, -12];
    // console.log(width)
    projection = d3.geoMercator()
        .center(mapCenter) //mapCenter
        .scale(mapScale) // 5100
        .translate([width / 3.1, height / .3]); // [horizontal, vertical]
    ...
    g.attr('id', 'countries')
        ...
        .attr('class', function(d) {
            var className = (countriesISO3Arr.includes(d.properties.ADM2_PCODE)) ? 'hasData' : 'inactive';
            return className;
        })
        .on("click", function(d) {
            ...
            countrySelectedFromMap = d.properties.ADM2_PCODE;
            countrySelectedFromMapName = d.properties.ADM2_EN;
...
function choroplethMap(mapData = filteredMappingData) {
    ...
    mapsvg.selectAll('path').each(function(element, index) {
        d3.select(this).transition().duration(500).attr('class', function(d) {
            var className = (countriesArr.includes(d.properties.ADM2_PCODE)) ? 'hasData' : 'inactive';
            return className;
        });
        d3.select(this).transition().duration(500).attr('fill', function(d) {
            var filtered = data.filter(pt => pt.key == d.properties.ADM2_PCODE);
